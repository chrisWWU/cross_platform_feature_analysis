...
def get_embeddings(path_connect, path_from, path_lookup, flickr):
    ...
    model = models.LINE(G, embedding_size=128, order='all')  # init model,order can be ['first','second','all']
    model.train(batch_size=1024, epochs=50, verbose=2)  # train model
    embeddings = model.get_embeddings()  # get embedding vectors

    res = np.zeros((len(embeddings), 256))
...
if __name__ == '__main__':
    ...
    path_to_fl = f'../../../features/{dataset}/friends/line_flickr_core.csv'
    path_to_tw = f'../../../features/{dataset}/friends/line_twitter_core.csv'
    csv = False
