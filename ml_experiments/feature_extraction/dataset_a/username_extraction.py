...
def execute(path_connect, min_gram, max_gram, min_df, max_features, analyzer, path_to_train, path_to_test, flickr, csv):
    ...
    if flickr:
        X_train_grams, X_test_grams = get_ngrams(fl_username, tw_username, min_gram, max_gram, min_df, max_features,
                                                 analyzer, 'user_')
        X_train_base, X_test_base = get_base_features(fl_username, tw_username)
    else:
        X_train_grams, X_test_grams = get_ngrams(tw_username, fl_username, min_gram, max_gram, min_df, max_features,
                                                 analyzer, 'user_')
...
if __name__ == '__main__':
    csv = False
    flickr = True  # TRUE -> ngrams learned from flickr, FALSE -> ngrams learned from twitter
    ...
    path_to_train = f'../../features/dataset_a/voc_learned_{init_voc}/username_train.csv'
    path_to_test = f'../../features/dataset_a/voc_learned_{init_voc}/username_test.csv'
