...
def get_embeddings(path_connect, path_from, path_lookup, flickr):
    ...
    model = models.SDNE(G, hidden_size=[256, 512])  # init model
    model.train(batch_size=3000, epochs=40, verbose=2)  # train model
    embeddings = model.get_embeddings()  # get embedding vectors

    res = np.zeros((len(embeddings), 512))
...
if __name__ == '__main__':
    ...
    path_to_fl = f'../../../features/{dataset}/friends/sdne_flickr_core.csv'
    path_to_tw = f'../../../features/{dataset}/friends/sdne_twitter_core.csv'
    csv = True
