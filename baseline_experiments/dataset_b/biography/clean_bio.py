...
def clean_bios(bios):
    bios = [delete_links(x) for x in bios]
    bios = [remove_URL(x) for x in bios]
    bios = [cleanhtml(x) for x in bios]
    bios = [clean_string2(x) for x in bios]
    bios = [tolower(x) for x in bios]
    return bios